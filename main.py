...
@bot.command(help="<предмет> <кількість*> <користувач*>")
...
@bot.command(help="<предмет> <користувач*>")
...
@bot.command(help="<продавець*> <предмет*> <мінімальна ціна*> <максимальна ціна*> <мінімальна кількість*>")
...
@bot.command(help="<предмет> <кількість> <ціна>")
...
@bot.command(help="<сума> <отримувач>")
async def send(ctx, amount, member: Member = None):
    if db.send_money(ctx.author.id, member.id, int(amount)):
        await ctx.send(f"Ви відправили {amount} кредитів користувачу {member.display_name}.")
    else:
        await ctx.send("Недостатньо коштів на рахунку.")
